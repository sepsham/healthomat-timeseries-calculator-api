...
namespace healthomat_timeseries_calculator_api.Controllers
{
    ...
    public class TimeSeriesController : BaseController
    {
        ...
        [HttpGet]
        [Route("GetAll")]
        public async Task<ActionResult<ApiResponse<List<TimeSeriesDto>>>> Get(CancellationToken cancellationToken = default)
        {
            try
            {
                var result = await _mediator.Send(new GetAllTimeSeriesQuery(), cancellationToken);
