...
namespace healthomat_timeseries_calculator_api.Controllers
{
    ...
    public class ZTimeSeriesController : BaseController
    {
        ...
        [HttpGet]
        [Route("GetAll")]
        public async Task<ActionResult<ApiResponse<List<ZTimeSeriesDto>>>> Get(CancellationToken cancellationToken = default)
        {
            try
            {
                var result = await _mediator.Send(new GetAllZTimeSeriesQuery(), cancellationToken);
